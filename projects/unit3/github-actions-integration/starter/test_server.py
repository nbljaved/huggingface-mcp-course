...
try:
    from server import mcp, analyze_file_changes, get_pr_templates, suggest_template
...
class TestImplementation:
    ...
    def test_imports(self):
        ...
        assert IMPORTS_SUCCESSFUL, (
            f"Failed to import required functions: {IMPORT_ERROR if not IMPORTS_SUCCESSFUL else ''}"
        )
        assert mcp is not None, "FastMCP server instance not found"
        assert callable(analyze_file_changes), (
            "analyze_file_changes should be a callable function"
        )
